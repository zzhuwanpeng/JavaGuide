...
## 核心知识点
红黑树是log(n),且算法稳定
负载因子*容量到0.75后，会进行rehash
头插法和尾插法：尾插法效率稍低，但头插法并发会存在死循环。
循环链表的形成：
  如果线程A和B在插入自己的节点后，错误地将对方新插入的节点设置为自己的下一个节点，就会形成一个循环链表。
  例子：A,B两个线程，A插入节点N1，B插入节点N2,有可能编程N1.next=N2, N2.next = N1
并发下还存在，丢失节点的问题，因为：
两个线程同时插入时，比如都想在通0插入一个新节点，相互不知道，就丢了。
