...
mysql默认是RR级别。    
MVCC：     
+ 快照读：如果读取的行正在执行 DELETE 或 UPDATE 操作，这时的读称为快照读
+ 当前读：在锁定读下，读取的是数据的最新版本，这种读也被称为 当前读（current read）。锁定读会对读取到的记录加锁：select ... lock in share mode：对记录加 S 锁，其它事务也可以加S锁，如果加 x 锁则会被阻塞select ... for update、insert、update、delete：对记录加 X 锁，且其它事务不能加任何锁

数据创建过程：
+ 创建版本：每当数据被修改时，系统会创建一条新的数据版本（行记录），而不是直接覆盖旧数据。
+ 事务ID：每个数据版本都会关联一个创建该版本的事务ID（Transaction ID）。
+ 隐藏列：在数据版本中，通常会有额外的隐藏列来存储版本信息，如事务ID、删除标记（用来标识哪个事务删除了该记录）和回滚指针（用来链接到旧的数据版本）。
+ 版本链：每个数据版本通过回滚指针形成一个版本链，使得系统可以根据事务的隔离级别和事务ID来找到合适的数据版本。
+ 可见性判断：事务在访问数据时，会根据自己的事务ID和隔离级别，通过可见性规则来判断哪个版本的数据是可见的。
